...
class Address(UnicodeProperty):
    ...
    """
    _attrs = ('summary', 'address', 'city', 'zip_code', 'phone', 'email', 'bank_account_name',
              'bank_name', 'bank_account', 'bank_code', 'note', 'vat_id', 'ir',
              'logo_filename', 'vat_note', 'country', 'division')

    def __init__(
        self, summary, address='', city='', zip_code='', phone='', email='', bank_account_name = '',
        bank_name='', bank_account='', bank_code='', note='', vat_id='', ir='',
        logo_filename='', vat_note='', country='', division='',
    ):
...
class Item(object):
    ...
    def __init__(self, count, price, discount = 0.0, description='', unit='', tax=Decimal(0), note='', credit_flag=False):
        self.count = count
        self.price = price
    ...
    def total_tax(self):
        ...
        return (self.price * self.count - self.discount) * (Decimal(1) + self.tax / Decimal(100))
...
class Invoice(UnicodeProperty):
    ...
    vat = Decimal(0.0)
    #: this installment
    this_installment = Decimal(0.0)
    #: this credit
    credit_spending = Decimal(0.0)
    ...
    def price(self):
        ...
        return self._round_result(sum(item.total for item in self.items), this_round_flag=True)
    ...
    def discount(self):
        ...
        return self._round_result(total_discount)
    ...
    def _round_price(self, price):
        return decimal.Decimal(price).quantize(Decimal('.01'), rounding=self.rounding_strategy)
    ...
    def _round_result(self, price, this_round_flag = False):
        if self.rounding_result or this_round_flag:
...
class Correction(Invoice):
    ...
    Correcting invoice
    """
    _attrs = ('number', 'document_type', 'reason', 'title', 'variable_symbol', 'specific_symbol', 'paytype',
